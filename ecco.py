...
class Lexer(object):
	tokens = [ 'NOTE', 'IS', 'OF', 'FLOAT', 'FRACTION', 'INTEGER', 'HERTZ', 
		...
		'LPAREN', 'LOAD', 'FILENAME', 'PRINT', 'ON', 'OFF', 'END_STMT' ]
...
class Parser(object):
	...
	def p_definitions2(self, p): #define note by pure interval
	...
	def p_definitions3(self, p): #define note by pure interval and beats per second
	...
	def p_definitions4(self, p): #define note by pure interval and fraction of a comma
	...
	def p_definitions5(self, p): #define note by pure interval and integer comma
	...
	def p_definitions6(self, p): #define note by pure interval and cents
	...
	def p_definitions7(self, p): #define note by cent
	...
	def p_definitions8(self, p): #drop all definitions
		"definition : DROP END_STMT"
		self.d.clear()
		self.lista()

	def p_definitions9(self, p): #
		...
		for a in range(1, 15):
			for b in range(1, 15):
				if abs(self.d[p[2]][0]*a - self.d[p[3]][0]*b) < t[2]:
					...
					t[0] = a
					t[1] = b
		print("  %s %.3f hertz and %s %.3f hertz are %.3f cents apart, beat at %.1f hertz at %d harmonic and %d harmonic" % (p[2], self.d[p[2]][0], p[3], self.d[p[3]][0], dif_cents, t[2], t[0], t[1]))
	...
	def p_definitions19(self, p):
		...
		if p[2] == "on":
			...
			raise Exception("PRINT desconhecido")
	...
	def parse_cents(self, cn, ab):
		if ab == "above" or ab == "up":
			...
			raise Exception("ABOVEBELOW desconhecido")
	...
	def parse_plusminus(self, pm, un):
		if pm == "plus":
			...
			raise Exception("PLUSMINUS desconhecido")
